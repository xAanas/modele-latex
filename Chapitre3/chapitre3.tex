...
Entity : il regroupe les entités de notre application.
Manager : il regroupe les éléments gérant la persistance dans la base de données.
Repository : il regroupe les éléments responsable de l'extraction des données de la base de données.
Views : il regroupe les vues de notre application. Il représente le contacte directe avec l'utilisateur.
Controller : il regroupe les contrôleurs de l'application. Ils jouent le rôle de chef d'orchestre en appellant le modèle et en le redirigent vers la vue convenue.
Il existe aussi d'autre packages :
Service : il regroupe les services liés à notre application.
Form : il regroupe les formulaires de l'application modélisés par des classes.
Config : il contient le routage de l'application et les services.

\subsection{Diagramme de classes}
\begin{itemize}
  \item Le modèle du domaine.\\
...
Dans le tableau 3.6, nous représentons le rôle de chacune de ces entités.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{|l|p{11cm}|}
    ...
    \hline
    Fichier & Il peut être un fichier attaché à une demande, une image dans commentaire ou l'image d'un utilisateur. \\
    \hline
    ...
    \hline
    Catégorie & La catégorie d'une demande sert dans la facturation. \\
    \hline
...
\end{table}
 ...
 \item Diagramme de classe des contrôleurs.\\
 La figure \ref{ctrlClass} montre le diagramme de classe des contrôleurs.
...
\begin{itemize}
  \item Diagramme de séquence du cas : ajouter une demande. \\
...
Lors de la réponse, DemandeController invoque le moteur de vue TWIG pour l'affichage de la demande dans l'accueil.
  ...
  \item Diagramme de séquence du cas : suppression d'un site
...
\caption{Diagramme d'activié : envoie d'une notification}
\label{envNotAct}
\end{figure} 

\section*{Conclusion}
Dans ce chapitre, nous avons préparer nos packages, nos diagrammes, des séquences d'utilisation, donc tout ce qu'on a besoin pour l'élaboration de l'application. Dans le chapitre suivant, nous allons entrer dans le vive du sujet et réaliser le projet.
